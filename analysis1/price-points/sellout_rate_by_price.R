...
sellout_rate_by_price_band = folkschool23_26combined_1_ %>%
  mutate(
  ...
  summarise(
    class_instances = n(),
    sellouts = sum(sellout, na.rm = TRUE),
    sellout_rate = sellouts / class_instances,
    .groups = "drop"
  ) %>%
  mutate(
    label = paste0(percent(sellout_rate, accuracy = 0.1), " • n=", comma(class_instances))
  )

p = ggplot(sellout_rate_by_price_band, aes(x = price_band, y = sellout_rate)) +
  ...
  labs(
    x = "Price band",
    y = "Sellout rate",
    title = "Sellout Rate by Price Band",
    subtitle = "Sellout = registrants >= max seats; labels show sellout rate and class count"
  ) +
...
ggsave(
  filename = "~/Desktop/Project/allerton-project/analysis1/price-points/sellout_rate_by_price_band.png",
  plot = p,
  width = 8,
  height = 5,
  dpi = 300
