...
waitlist_by_price_band = folkschool23_26combined_1_ %>%
  mutate(
    price_num = as.numeric(price)
  ) %>%
  group_by(class, class_begins, price_num, waiting_list) %>%
  ...
  mutate(
    label = paste0(number(avg_waitlist, accuracy = 0.1), " • n=", comma(class_instances))
  )

p = ggplot(waitlist_by_price_band, aes(x = price_band, y = avg_waitlist)) +
  ...
  labs(
    x = "Price band",
    y = "Average waitlist",
    title = "Average Waitlist by Price Band",
    subtitle = "Labels show average waitlist and class count"
  ) +
...
ggsave(
  filename = "average_waitlist_by_price_band.png",
  plot = p,
  width = 8,
  height = 5,
  dpi = 300
